...
RATE_LIMIT_SEC = 0.12  # pykrx rate limit
...
def get_last_date_in_history() -> str | None:
    ...
    if not os.path.exists(PATHS["HISTORY"]):
        return None
    try:
        df = pd.read_csv(PATHS["HISTORY"], dtype={"code": str}, encoding="utf-8-sig")
...
def determine_date_range(args, last_date: str | None) -> tuple[str, str]:
    """수집 범위 결정 (YYYYMMDD, YYYYMMDD)."""
    today_kst = datetime.now(KST).date()
    # 어제까지 (오늘 데이터는 장 마감 후라야)
...
def main():
    ...
    for i, m in enumerate(stocks_meta, 1):
        ...
        # OHLCV
        df_o = fetch_ohlcv(code, start, end)
        if not df_o.empty:
            df_o["name"] = name
