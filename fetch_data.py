...
def fetch_ohlcv_with_cap(code: str, start: str, end: str) -> tuple[pd.DataFrame, dict]:
    ...
    # 1. OHLCV (한글/영문 컬럼 모두 대응 + 거래대금 fallback)
    try:
        ...
        if not df.empty:
            df = df.reset_index().rename(columns={
                ...
                "등락률": "change_rate",
                # 영문 컬럼 (인증 모드 — 거래대금은 'Amount')
                "Date": "date", "Open": "open", "High": "high",
                "Low": "low", "Close": "close",
                "Volume": "volume", "Amount": "trade_value",
            })
            # 거래대금 fallback: 컬럼이 없으면 종가×거래량으로 직접 계산
            # (KRX_ID 인증 모드는 응답에 거래대금 컬럼이 없음)
